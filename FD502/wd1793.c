...
DWORD dwRet;
FILE* OpenFloppy (int );
static void *RawReadBuf=NULL;

bool SetDataRate (FILE * , u_int8_t );
...
u_int64_t				DataLength = sizeof(KEYBOARD_INDICATOR_PARAMETERS);
...
unsigned char MountDisk(char *FileName,unsigned char disk)
{
	...
	if (Drive[disk].RawDrive==0)
	{
		...
		fseek(Drive[disk].FileHandle,0,SEEK_END);
		Drive[disk].FileSize=ftell(Drive[disk].FileHandle);
		Drive[disk].HeaderSize = Drive[disk].FileSize % 256;
		fseek(Drive[disk].FileHandle,0,SEEK_SET);
		BytesRead = fread(HeaderBlock, 1, HEADERBUFFERSIZE, Drive[disk].FileHandle);
	}
	else
...
//************************
	...
	if (Drive[CurrentDisk].FileHandle==NULL)
		return(0);
	...
	switch (Drive[CurrentDisk].ImageType)
	{
		...
		case OS9:
			...
			fseek(Drive[CurrentDisk].FileHandle,FileOffset,SEEK_SET);
			BytesRead = fread(ReturnBuffer, 1, BytesperSector[Drive[CurrentDisk].SectorSize], Drive[CurrentDisk].FileHandle);
		...
		case RAW:
			...
			//{
				...
				//sp.head = Side;
				// seek to cyl 
				//DeviceIoControl(Drive[CurrentDisk].FileHandle , IOCTL_FDCMD_SEEK, &sp, sizeof(sp), NULL, 0, &dwRet, NULL);
				//Ret=DeviceIoControl(Drive[CurrentDisk].FileHandle , IOCTL_FDCMD_READ_DATA, &rwp, sizeof(rwp), RawReadBuf,4608, &dwRet, NULL);
				if (dwRet != 4608)
					return(0);
				DirtyDisk=0;
			//}
...
//Needed for RAW access
	...
	unsigned char *pva=NULL;
	SectorInfo CurrentSector;
	if ( (Drive[CurrentDisk].FileHandle==NULL) | ((Side+1) > Drive[CurrentDisk].Sides) )
		return(0);
	...
	switch (Drive[CurrentDisk].ImageType)
	{
		...
		case OS9:
			FileOffset= Drive[CurrentDisk].HeaderSize + ( (Track * Drive[CurrentDisk].Sides * Drive[CurrentDisk].TrackSize)+ (Side * Drive[CurrentDisk].TrackSize) + (BytesperSector[Drive[CurrentDisk].SectorSize] * (Sector - Drive[CurrentDisk].FirstSector) ) ) ;
			Result=fseek(Drive[CurrentDisk].FileHandle,FileOffset,SEEK_SET);
			BytesWritten = fwrite(WriteBuffer,1,BytesperSector[Drive[CurrentDisk].SectorSize],Drive[CurrentDisk].FileHandle);
			return(BytesWritten);
		break;
		...
		case OS9:
			for (BufferIndex=0;BufferIndex<6272;BufferIndex++)
			{
				...
				if ( WriteBuffer[BufferIndex]== 0xFB)	//Look for D.A.M.
				{
					...
					Result=fseek(Drive[CurrentDisk].FileHandle,FileOffset,SEEK_SET);
					BytesWritten = fwrite(&WriteBuffer[BufferIndex],1,BytesperSector[Drive[CurrentDisk].SectorSize],Drive[CurrentDisk].FileHandle);
...
void PingFdc(void)
{
	...
	if ( ((CurrentCommand<=7) | (CurrentCommand==IDLE)) & (Drive[CurrentDisk].FileHandle!=NULL) )
	{
		if (IndexPulse  )
	...
	switch (CurrentCommand)
	{
		case RESTORE:	
			if (Drive[CurrentDisk].HeadPosition!=0)
			{
				...
				//if (Drive[CurrentDisk].ImageType==RAW)
					//DeviceIoControl(Drive[CurrentDisk].FileHandle , IOCTL_FDCMD_SEEK, 0, 1, NULL, 0, &dwRet, NULL);
			}
			else
			{
				TrackReg=0;
		...
		case SEEK:	
			...
			if (Drive[CurrentDisk].ImageType==RAW)
			{
				//DeviceIoControl(Drive[CurrentDisk].FileHandle , IOCTL_FDCMD_SEEK, &TrackReg, sizeof(TrackReg), NULL, 0, &dwRet, NULL);
			}
		break;
...
//	MessageBox(0,szDevice,"ok",0);
    ...
    return (h != NULL && SetDataRate(h, DISK_DATARATE)) ? h : NULL;
}

bool SetDataRate (FILE *h_, u_int8_t bDataRate_)
{
...
//     return 0;//!!DeviceIoControl(h_, IOCTL_FDCMD_FORMAT_TRACK, pfp_, ulSize_, NULL, 0, &dwRet, NULL);
// }
